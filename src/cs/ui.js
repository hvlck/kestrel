...
function listen(event) {
    if (event.keyCode == 13 && commandList) {
        ...
        if (commandIndex <= 0) {
            commandList.children[commandList.children.length - 1].classList.add(
            ...
            commandIndex -= 1;
            commandList.children[commandIndex].classList.add(
                "kestrel-command-item-focused"
            );
            commandList.scrollTo(
                0,
                parseInt(
                    getComputedStyle(commandList.children[commandIndex]).height
                ) * commandIndex
            );
        }
        ...
        if (commandIndex >= commandList.children.length - 1) {
            commandList.children[0].classList.add(
                "kestrel-command-item-focused"
            );
            commandIndex = 0;
